    ...
    <div class="animation w-full h-full z-20">
        <div class="nav-nav fixed px-4 py-4 sm:px-8 sm:py-5 top-0 left-0 h-16 z-30 flex items-center dark:bg-gray-700 bg-white justify-between  w-full border-b-2 border-gray-600 dark:border-gray-200">
            <div class="w-1/2 flex items-center">
                <h1 class="text-title dark:text-gray-100 stroke-1 font-bowlby add_stroke text-3xl">Note Me</h1>
            </div>
            <div class="w-5/12 flex justify-between items-center text-dark-black dark:text-gray-100">
                ...
                <div class="flex items-center justify-evenly" :class="locale==='mm'?'text-md w-6/12':'text-xl w-5/12 '">
                    <button
                        ...
                        :class="locale==='mm'?'font-extrabold':'add_stroke'" @click="ShowLogin()">{{ messages.login }}
                    </button>
                    <button
                        class="focus:outline-none px-2 py-1 font-bowlby text-signup tracking-wider dark:text-gray-100 "
                        :class="locale==='mm'?'font-extrabold':'add_stroke'" @click="ShowSignUp()">
                        {{ messages.signup }}
                    </button>
                </div>
            </div>
        ...
        <div class="w-full h-full relative items-center justify-center flex flex-col">
            <div class="h-full w-full overflow-hidden">
                <div class="holder h-full w-full transform translate-y-full pt-20 p-4">
                    <div v-show="isAbout"
                         class="about w-full dark:text-gray-100 font-poppins  h-full bg-white dark:bg-gray-800 flex items-center flex-col border-2 border-gray-600 dark:border-gray-200">
                    ...
                    <div v-show="isReadMe"
                         class="read_me w-full h-full bg-white dark:bg-gray-800 flex items-center flex-col border-2 border-gray-600 dark:border-gray-200">
                        <svg xmlns="http://www.w3.org/2000/svg" @click="hide()"
                             class="self-end mt-1 mr-1 cursor-pointer w-12 text-gray-600 stroke-current"
                             viewBox="0 0 24 24">
                            <path fill="#6563ff"
            ...
            <div class="w-full bottom-0 left-0 absolute flex justify-center items-center overflow-hidden">
                <div class="navigation w-9/12 flex h-24 sm:h-32 md:h-1/6 py-4 justify-evenly font-poppins">
                    <div class="w-3/12 bg-about px-2 flex items-center justify-center py-4 border-white  border-2"><span
                        ...
                        @click="About">About</span></div>
                    <div class="w-3/12 bg-read_me px-2 flex items-center justify-center py-4 border-white  border-2">
                        <span class="add_stroke_white stroke-current tracking-widest cursor-pointer" @click="ReadMe">Read Me</span>
                    </div>
                    <div class="w-3/12 bg-contact_me px-2 flex items-center justify-center py-4 border-white  border-2">
                        <span class="add_stroke_white stroke-current tracking-widest cursor-pointer"
                              @click="ContactMe">Contact Me</span></div>
                </div>
            </div>
        </div>
    </div>
